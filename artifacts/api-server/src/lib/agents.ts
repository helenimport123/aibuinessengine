...
/** Callback fired when CEO creates orchestrated sub-tasks */
export type OnTasksCreated = (tasks: Array<{
  id: number;
  agentType: string;
  agentName: string;
  projectId: number;
  ...
  try {
    if (project.userId) {
      ...
      if (!budgetRes.allowed) {
        throw new Error(`Ngân sách tháng đã hết: đã dùng $${budgetRes.spent.toFixed(4)} / $${budgetRes.limit.toFixed(2)}`);
      }
      if (!quotaRes.allowed) {
        throw new Error(`Quota token hàng ngày đã hết: ${quotaRes.used.toLocaleString()} / ${quotaRes.limit.toLocaleString()} tokens`);
      }
    }
      ...
      try {
        ...
        if (executionPlan.length > 0) {
          ...
          if (onTasksCreated && createdTasks.length > 0) {
            await onTasksCreated(createdTasks.map((t) => ({
              id: t.id,
    ...
    if (agentType === "ceo") {
      await db.update(projectsTable).set({ status: "draft" }).where(eq(projectsTable.id, projectId));
    }
